...
use crate::protocol::{
    FileChange, FileStatus, Node, Region, Source, SourcePos, SourceRange, Visibility,
};
...
impl FoldMutation for DeletedBodies {
    fn apply(&self, _file: &FileChange, sides: &mut Pairing<Source>) -> anyhow::Result<()> {
        let (lhs, rhs_ids) = match sides {
            Pairing::Both { lhs, rhs } => (lhs, ids(&rhs.regions)),
            Pairing::LeftOnly { lhs } => (lhs, DftHashSet::default()),
            Pairing::RightOnly { .. } => return Ok(()),
        };
...
mod tests {
    ...
    fn removed_runs_respect_the_threshold_and_the_switch() {
        ...
        assert_eq!(lhs_side.regions.len(), 1);
