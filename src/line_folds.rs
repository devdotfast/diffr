...
//! `parse::folds::project`). A line diff has no matcher, so folds pair
...
/// Pair the folds of a line-diff fallback: both folds of a pair record the
/// other as their `partner`.
///
...
pub(crate) fn pair(
    ...
    for (lhs_fold, rhs_index) in lhs_folds.iter_mut().zip(lhs_pair) {
        let Some(rhs_index) = rhs_index else {
            continue;
        };
        let rhs_fold = &mut rhs_folds[rhs_index];
        lhs_fold.partner = Some(rhs_fold.syntax_id);
        rhs_fold.partner = Some(lhs_fold.syntax_id);
    }
}
...
fn spans(folds: &[Fold], line_count: usize) -> Vec<Option<(usize, usize)>> {
    folds
        .iter()
        .map(|fold| {
            let (start, end) = folds::line_span(fold, line_count);
