...
//! sequence: deleted function bodies, test bodies, removed runs, docstring
//! bundling, the built-in summarizer, the JSON-RPC hook, docstring
//! visibility, and last the grouping pass that merges adjacent gaps and
//! wraps runs of collapsed folds. A mutation
...
use crate::protocol::{FileChange, Node, Region, Source, Visibility};
use std::fmt;
use std::path::Path;

pub(crate) trait FileMutation: Send + Sync {
    fn apply(&self, file: &mut FileChange) -> anyhow::Result<()>;
}
...
/// Which mutation stopped the run. The failing mutation attaches it to its
/// error, and the stream turns it into `complete.aborted.code`.
...
impl Failure {
    pub(crate) fn code(self) -> &'static str {
        match self {
            Self::Summarizer => "summarizer_failed",
            Self::Hook => "hook_failed",
        }
    }
}
...
impl Mutations {
    ...
    pub(crate) fn from_params(params: &Params, workspace: &Path) -> anyhow::Result<Self> {
        ...
        if folds.bundle_docstrings {
            mutations.fold.push(Box::new(docstrings::Docstrings));
        }
        ...
        if folds.bundle_docstrings {
            mutations
                .fold
                .push(Box::new(docstrings::DocstringVisibility));
        }
        mutations.fold.push(Box::new(group::GroupCollapsed));
        Ok(mutations)
    }

    pub(crate) fn apply_file(&self, file: &mut FileChange) -> anyhow::Result<()> {
