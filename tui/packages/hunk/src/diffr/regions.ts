...
/** A leaf tiles its side; the same id on the other side is its counterpart. */
...
export function flattenSide(source: Source, side: Side): { leaves: Leaf[]; folds: Fold[] } {
  ...
  const visit = (region: Region, ancestors: Fold[]) => {
    if (region.kind === "leaf") {
      ...
      leaves.push({ id: region.id, side, startLine, endLine, changed, tags: region.tags,
    ...
    for (const ancestor of ancestors) ancestor.nested.push(fold.id);
    folds.push(fold);
...
export function defaultCollapsed(diff: TextDiff): Set<number> {
  ...
  for (const item of [...leaves.flat(), ...folds.flat()]) if (item.collapsed) ids.add(item.id);
  return ids;
}
...
export function foldIds(diff: TextDiff): number[] {
  ...
  return [...new Set([...folds.flat().map((fold) => fold.id), ...leaves.flat().filter(foldableLeaf).map((l) => l.id)])];
}
...
export function nestedIds(diff: TextDiff, id: number): number[] {
  const fold = flatten(diff).folds.flat().find((f) => f.id === id);
  if (fold) return fold.nested;
  if (!flatten(diff).leaves.flat().some((leaf) => leaf.id === id)) throw new Error(`Unknown region ${id}`);
  return [];
}
/** Context-gap leaves: unchanged runs diffr trimmed to N lines around changes. */
export function gapIds(diff: TextDiff): number[] {
  return [...new Set(flatten(diff).leaves.flat().filter((leaf) => leaf.tags.includes("unchanged")).map((l) => l.id))];
}
...
export function hiddenLines(folds: Fold[], collapsed: ReadonlySet<number>): Set<number> {
  ...
  for (const fold of ordered) {
    if (!collapsed.has(fold.id) || hidden.has(fold.headerLine)) continue;
...
export function foldHeaders(folds: Fold[], leaves: Leaf[], collapsed: ReadonlySet<number>): Map<number, RowFold> {
  ...
  for (const leaf of leaves)
    if (foldableLeaf(leaf))
      headers.set(leaf.startLine, { id: leaf.id, label: leafLabel(leaf), collapsed: collapsed.has(leaf.id) });
  for (const fold of folds) {
    const existing = headers.get(fold.headerLine);
    const existingFold = existing && folds.find((f) => f.id === existing.id);
    if (existingFold && existingFold.lastHidden >= fold.lastHidden) continue;
    headers.set(fold.headerLine, { id: fold.id, label: fold.label, collapsed: collapsed.has(fold.id) });
  }
  return headers;
}
