...
test("rows lead with titles under group headings; toggles change in place and typed values open a prompt", async () => {
  ...
  try {
    ...
    expect(line("Shortest body to collapse")).not.toContain("folds.min_lines");
    expect(frame()).toContain("folds.min_lines · default 12");
    ...
    await act(async () => { await t.renderOnce(); });
    expect(writes.at(-1)).toEqual(["summarize.api_key", "abcd"]);
