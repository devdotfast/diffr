...
use crate::protocol::{FileChange, Node, Region, Source, SourceRange, Visibility};
...
/// An id above every region id in the file: the projection's dense ids
/// and any ids earlier mutations minted.
...
fn merge_gaps(sides: &mut Pairing<Source>) {
    let (lhs_adjacent, rhs_adjacent, lhs_ids, rhs_ids) = {
        ...
        let (lhs_ids, rhs_ids) = match &*sides {
            Pairing::Both { lhs, rhs } => {
                adjacent_gaps(&lhs.regions, &mut lhs_adjacent);
                adjacent_gaps(&rhs.regions, &mut rhs_adjacent);
                (ids(&lhs.regions), ids(&rhs.regions))
            }
            Pairing::LeftOnly { lhs } => {
                adjacent_gaps(&lhs.regions, &mut lhs_adjacent);
                (ids(&lhs.regions), DftHashSet::default())
            }
            Pairing::RightOnly { rhs } => {
                adjacent_gaps(&rhs.regions, &mut rhs_adjacent);
                (DftHashSet::default(), ids(&rhs.regions))
            }
        };
        (lhs_adjacent, rhs_adjacent, lhs_ids, rhs_ids)
    };
...
fn group_folds(sides: &mut Pairing<Source>, next_id: &mut u32) {
    ...
    match &*sides {
        Pairing::Both { lhs, rhs } => {
            collapsed_fold_runs(&lhs.regions, &ids(&rhs.regions), &mut lhs_runs);
            collapsed_fold_runs(&rhs.regions, &ids(&lhs.regions), &mut rhs_runs);
        }
        Pairing::LeftOnly { lhs } => {
            collapsed_fold_runs(&lhs.regions, &DftHashSet::default(), &mut lhs_runs);
        }
        Pairing::RightOnly { rhs } => {
            collapsed_fold_runs(&rhs.regions, &DftHashSet::default(), &mut rhs_runs);
        }
    }
...
mod tests {
    use super::*;
    use crate::mutate::collapse::{DeletedBodies, TestBodies};
    ...
    fn removed_groups_only_wrap_one_sided_folds() {
        ...
        let (Pairing::Both { lhs: lhs_side, .. } | Pairing::LeftOnly { lhs: lhs_side }) = &sides
        else {
            panic!("a before side");
        };
        let lhs = &lhs_side.regions;
        assert!(
    ...
    fn adjacent_deleted_bodies_are_grouped_under_one_collapsed_fold() {
        ...
        DeletedBodies { min_lines: 3 }
            ...
            .unwrap();
        GroupCollapsed.apply(&file, &mut sides).unwrap();
        let (Pairing::Both { lhs, .. } | Pairing::LeftOnly { lhs }) = &sides else {
            panic!("a before side");
        };
        let group = lhs
            .regions
        ...
        let rhs_ids = super::super::ids(&rhs_side.regions);
        assert!(
    ...
    fn a_single_collapsed_fold_is_left_alone() {
        ...
        walk(&lhs_side.regions, &mut |region| {
    ...
    fn adjacent_gaps_merge_on_both_sides_and_keep_their_pairing() {
        ...
        for source in [lhs_side, rhs_side] {
    ...
    fn gaps_do_not_merge_when_the_other_side_separates_them() {
        ...
        assert_eq!(lhs_side.regions.len(), 2);
        let (Pairing::Both { rhs: rhs_side, .. } | Pairing::RightOnly { rhs: rhs_side }) = &sides
        else {
            panic!("an after side");
        };
        assert_eq!(rhs_side.regions.len(), 3);
    }
