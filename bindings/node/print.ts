import type { RegionData as Region, SearchResultData, SourceData as Source } from "./types.ts";
...
export function print(result: SearchResultData): string {
  const lhs = result.display !== "rhs" ? result.sources.same ?? result.sources.lhs : undefined;
  const rhs = result.display !== "lhs" ? result.sources.same ?? result.sources.rhs : undefined;
  ...
  const unchanged = Boolean(result.sources.same);
  ...
  if (combined) {
    ...
    // Shared content has one tree; display determines its side labels.
