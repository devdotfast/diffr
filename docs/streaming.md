...
- On a `file` record, `error` replaces `diff` and the run continues. Codes:
  ...
  `unmerged`, `read_failed`, `fold_pairing`, or `internal` for a failure diffr
  did not classify.
...
Ids are per file and dense from 0. The projection assigns them as it builds
the regions, in lhs preorder then rhs preorder; a region paired with one on
the other side takes that region's id instead of a new one. Mutations take
ids above every id already in the file. Ids mean nothing across files or
runs.
...
The static viewer under `examples/review/viewer` predates this stream and does
not read it. Its build script captures the pinned Git patches only.
