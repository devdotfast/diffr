...
/** Keys match fuzzily; descriptions only as whole substrings, so prose does not swamp the list. */
function settingScore(setting: Setting, query: string): number | null {
  const byKey = fuzzyScore(query, setting.key);
  const byDescription = setting.description.toLowerCase().includes(query.toLowerCase()) ? query.length : null;
  if (byKey === null && byDescription === null) return null;
  return Math.max(byKey ?? -1, byDescription ?? -1);
}
export function filterSettings(settings: Setting[], query: string): Setting[] {
  return settings
    .map((setting) => ({ setting, score: settingScore(setting, query) }))
    .filter((entry): entry is { setting: Setting; score: number } => entry.score !== null)
    .sort((a, b) => b.score - a.score || a.setting.key.localeCompare(b.setting.key))
