...
Hydration retains the complete indexed comparison. It attaches selected highlights
and initially collapses regions without evidence. Repeated hits are deduplicated;
source trees remain ordered and non-overlapping. Postprocessing unions highlights
from compatible selections and applies plugins to these complete trees.
...
No stored `region_changed` or `highlights_changed` flags. Callers can rank/filter
whole results. To deselect a region, clear its descendant search highlights;
do not remove or reorder the complete comparison's regions.
...
`SearchResult` is the bound client envelope around a complete comparison. Each `Source` holds one side of one file;
...
FileRef contains path, oid, and mode. `sources.same` holds shared unchanged
content once, while `file` keeps both revision references. `display` controls
printing and Review presentation; it never changes plugin input. `both` means all
available sides; requesting an absent side rejects. `inspect` is Node's util symbol.
...
- Rust lazily indexes pinned Git blobs and caches structural analysis and
  ...
  separate scope-resolution call. Postprocess overrides configure mutation behavior;
  they do not rebuild the structural queries used by hydration.
- Hydration maps hits to the indexed source and attaches highlights. The current
  ...
  richer search input later. Distinct source counterparts are highlighted independently; Same has one shared tree.
...
- Combined versus side-only results use indexed correspondence, including renames,
  not filename equality. Unchanged content uses `sources.same`.
- Hydration initially exposes selected evidence without surrounding context.
- `postprocess` coalesces compatible selected comparisons, unions their highlights, and applies configured plugins for context and
...
- Hydrated and postprocessed sources retain the full indexed comparison.
...
- Plugins run on the complete original comparison, independently of `display`. Both ordinary diffs and search use the same plugin pipeline.
...
// 4. SELECT: rank whole file comparisons, preferring changed matches.
const selected = hydrated.sort((a, b) =>
  Number(Object.values(b.sources).some(s => s.regions.some(r => r.hasChangedHighlights()))) -
  Number(Object.values(a.sources).some(s => s.regions.some(r => r.hasChangedHighlights())))
).slice(0, 5);

// 5. POSTPROCESS: apply context/folding plugins to the complete comparisons.
...
### Original comparison at the plugin boundary

The plugin SDK `Pairing<Source>` and WIT `source-sides` gain `Same { source }` / `same(source)`. This represents identical source once, with ordinary regions, empty changed spans, and selected search highlights. It is neither added nor deleted. `Both`, `LeftOnly`, and `RightOnly` describe the original comparison, never display-side selection. File metadata retains the actual original refs, including both refs for `Same`.

Hydration owns highlight attachment and retains complete trees. Postprocessing
validates them against the pinned index and runs plugins directly on the original
comparison. Equivalent selections with different display sides share processing.
The JS result uses the Comparison union above; Review upgrades older stored evidence
on read without inventing missing sides. The computed-diff cache schema is unchanged.
The SDK and WIT change requires rebuilding external plugins.
