...
use crate::protocol::{FileChange, Node, Pairing, Problem, Region, Source, SourcePos, SourceRange};
...
impl FoldMutation for Docstrings {
    fn apply(&self, file: &FileChange, sides: &mut Pairing<Source>) -> Result<(), Problem> {
        ...
        for (source, docs) in [(sides.lhs(), &lhs_docs), (sides.rhs(), &rhs_docs)] {
...
mod tests {
    ...
    fn a_comment_separated_from_the_function_by_code_does_not_count() {
        ...
        Docstrings.apply(&file, &mut sides).unwrap();
        assert!(docstrings(sides.rhs().unwrap()).is_empty());
    }
